...
function getProfileWithData(profileId: string): BudgetProfile | null {
  ...
  const accounts = db.prepare('SELECT * FROM accounts WHERE profile_id = ?').all(profileId) as AccountRow[];
  const rules = db.prepare('SELECT * FROM budget_rules WHERE profile_id = ?').all(profileId) as BudgetRuleRow[];
...
profilesRouter.get('/', (_req, res) => {
  const profiles = db.prepare('SELECT * FROM profiles ORDER BY created_at DESC').all() as ProfileRow[];
  
  const profilesWithData = profiles.map(p => getProfileWithData(p.id)).filter(Boolean) as BudgetProfile[];
...
profilesRouter.get('/:id', (req, res) => {
  const profile = getProfileWithData(req.params.id);
...
profilesRouter.post('/', (req, res) => {
  ...
  const profile = getProfileWithData(id);
...
profilesRouter.post('/:id/duplicate', (req, res) => {
  const sourceProfile = getProfileWithData(req.params.id);
  ...
  const transaction = db.transaction(() => {
    ...
    db.prepare(
      'INSERT INTO profiles (id, name, created_at, updated_at) VALUES (?, ?, ?, ?)'
    ).run(newProfileId, `${sourceProfile.name} (Copy)`, timestamp, timestamp);
    ...
    for (const account of sourceProfile.accounts) {
      ...
      ).run(newAccountId, newProfileId, account.name, account.type, account.startingBalance, timestamp);
    }
    ...
    for (const rule of sourceProfile.rules) {
      ...
      db.prepare(
        ...
        newRuleId, newProfileId, rule.label, rule.amount, rule.type,
  ...
  const newProfile = getProfileWithData(newProfileId);
